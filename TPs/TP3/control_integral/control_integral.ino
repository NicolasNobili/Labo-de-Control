...
const float k[4] = {2,-0.0387,-0.6431,-0.3006};
const float h[2] = {2.7381e-5,2.7026};
...
float q_theta_actual = 0;
float q_theta_posterior = 0;
...
void loop() {
  ...
  // Delay para la respuesta al escalon:
  if(counter_step==0){
    r_phi = step_ref;
  }
  else{
  ...
  // ESTADO AUMENTADO:
  q_theta_posterior = CTRL_PERIOD_S * (r_theta - theta_monio_actual) + q_theta_actual;
  q_phi_posterior   = CTRL_PERIOD_S * (r_phi   -   phi_monio_actual) + q_phi_actual;


  // CONTROLADOR
  u = k[0]*theta_monio_actual + k[1]*theta_punto_monio_actual + k[2]*phi_monio_actual + k[3] * phi_punto_monio_actual + h[0] * q_theta_actual + h[1] * q_phi_actual;
    // Saturador
  if(u > u_max){
    u = u_max;  
  }else if(u < u_min){
  ...
  q_theta_actual = q_theta_posterior;

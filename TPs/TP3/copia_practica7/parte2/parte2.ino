...
float theta_monio_punto_actual = 0;
float theta_monio_punto_posterior = 0;
float b_actual = 0;
float b_posterior = 0;
float Ad[3][3] = {{0.9969,0.009972,0},{-0.6187,0.9933, 0},{0,0,1}};

bool  matlab = true;

//float L[3][2] = {{0.0434, 0.0002}, {-0.5790,0.0003},{-0.0531,0.0261}};  //x15

//float L[3][2] = {{1.1221, 0.0622}, {94.6245,-28.2106},{-95.7723, 29.2392}};  //x15

float L[3][2] = {{0.5821, -0.5350}, {8.0906,-16.2343},{-8.0116, 16.5632}};  //x15
...
void loop() {
  ...
  y1_monio = theta_monio_actual;
  y2_monio = theta_monio_punto_actual + b_actual;

  // Estimacion de theta y theta_punto en base a el observador de Lurenberg
  theta_monio_posterior = Ad[0][0] * theta_monio_actual + Ad[0][1] * theta_monio_punto_actual + L[0][0] * (y1 - y1_monio) + L[0][1]*(y2 - y2_monio);   
  theta_monio_punto_posterior = Ad[1][0] * theta_monio_actual + Ad[1][1] * theta_monio_punto_actual + L[1][0] * (y1 - y1_monio) + L[1][1]*(y2 - y2_monio);
  b_posterior = Ad[2][2]* b_actual + L[2][0] * (y1 - y1_monio) + L[2][1]*(y2 - y2_monio);

  if(matlab){
    // Junto los datos en un array y los envio por puerto serie  
    float data[5] = {theta_monio_actual, theta_f, theta_monio_punto_actual+b_actual,g.gyro.x, b_actual};
    serial_sendN(data,5);
  }
